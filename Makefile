...
export version := $(shell cat chenile-bdd-version.txt)
...
.PHONY: upgrade-new-version
upgrade-new-version:
	@ if ! test -n "$(newversion)"; then echo "newversion needs to be specified. Use 'make $(MAKECMDGOALS) newversion=<newversion>'"; exit 1 ; fi
	scripts/upgrade-new-version.sh $(newversion)
